...
- [ ] **Production has `077-expand` applied and verified, before an RC carrying the new COURT/Side call site is cut.** This is a shipping precondition, not a development one — the client may be wired against staging now.
  ...
  A stale 2-arg call has been observed failing as **PostgREST `PGRST202`** ("Could not find the function ... in the schema cache") *and* reported as **Postgres `42883`** (`undefined_function`). Both are plausible — PostgREST answers from its schema cache, Postgres raises when a call reaches it — so **any alert, log filter or client-side check must match either, never one**. Building on a single code is how this becomes invisible.
