...
## Device push — proven end to end (2026-08-19)

With the paid Apple Developer account in place, the whole push chain ran
on a real iPhone:

- Apple credentials generated by EAS — distribution certificate,
  provisioning profile carrying the device, and (the part a free account
  can never hold) an **Apple Push Notifications key** bound to
  `com.airrally.app`.
- The signed development build installed on the device. iOS 16+ requires
  **Settings → Privacy & Security → Developer Mode** for any development
  build; that is expected, not a defect.
- On sign-in the app requested permission, obtained a real Expo push
  token, and wrote it through `register_push_token` into
  `device_push_tokens` — `ExponentPushToken[6XEDG4ObJG…]`.
- A push sent through Expo returned ticket `ok`, and — the check that
  actually matters — the **delivery receipt** came back `ok`, meaning
  Apple accepted it. A ticket alone only proves Expo took the message.
- The notification arrived on the handset and its tap opened the app.

The one leg not yet connected in production: the database trigger posts
to `air-rally.com/api/webhooks/notification-push`, which 404s until the
web branch deploys. That leg was verified separately against staging
with a seeded token. Until the deploy, pushes must be sent manually;
after it, real bookings notify on their own.
